...
if not _G.AccentColors then
    _G.AccentColors = {
        Accent1 = Color3.fromRGB(240, 40, 40),
...
function library:CreateWindow(name, size, hidebutton)
    ...
    function window:CreateTab(name)
        ...
        function tab:CreateConfigSystem(side, filename, FolderName)
            local configSystem = { }

            configSystem.configFolder = FolderName
            ...
            local default = tostring(listfiles(configSystem.configFolder)[1] or ""):gsub(configSystem.configFolder .. "\\", ""):gsub(filename, "")
            ...
            for i,v in pairs(listfiles(configSystem.configFolder)) do
                if v:find(filename) then
                    Config:Add(tostring(v):gsub(configSystem.configFolder .. "\\", ""):gsub(filename, ""))
                end
            end

            configSystem.Create = configSystem.sector:AddButton("Create", function()
                for i,v in pairs(listfiles(configSystem.configFolder)) do
                    Config:Remove(tostring(v):gsub(configSystem.configFolder .. "\\", ""):gsub(filename, ""))
                end
                ...
                if ConfigName:Get() and ConfigName:Get() ~= "" then
                    ...
                    writefile(configSystem.configFolder .. "/" .. ConfigName:Get() .. filename, httpservice:JSONEncode(config))
            ...
            configSystem.Save = configSystem.sector:AddButton("Save", function()
                ...
                if Config:Get() and Config:Get() ~= "" then
                    ...
                    writefile(configSystem.configFolder .. "/" .. Config:Get() .. filename, httpservice:JSONEncode(config))
                end
            end)

            configSystem.Load = configSystem.sector:AddButton("Load", function()
                local Success = pcall(readfile, configSystem.configFolder .. "/" .. Config:Get() .. filename)
                if (Success) then
                    pcall(function() 
                        local ReadConfig = httpservice:JSONDecode(readfile(configSystem.configFolder .. "/" .. Config:Get() .. filename))
                        local NewConfig = {}
            ...
            configSystem.Delete = configSystem.sector:AddButton("Delete", function()
                ...
                if (not isfile(configSystem.configFolder .. "/" .. Config:Get() .. filename)) then return end
                delfile(configSystem.configFolder .. "/" .. Config:Get() .. filename)
